...
workflow_iobrServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # 2. Preprocessing Selection Logic
    ...
    # 3. Detect outlier
    res_outlier <- find_outlier_samplesServer(
      "mod_detcct_outlier",
      external_eset = res_preprocess
    )
    ...
    }) # 如果 outlier 模块没跑(NULL)，就回退用当前 preprocessing 结果
    ...
    sig_rocServer("mod_sig_roc", external_eset = res_combined)
